...
export function Storefront() {
  ...
  useEffect(() => {
    fetch("/api/shop")
      ...
      .then((d) => {
        ...
        setManagedCategories(d.categories ?? []);
      })
      .catch(() => {
        setProducts([]);
  ...
  // Group products by category name. Categories are ordered to match the
  // managed product_categories list from the API; any category not in that
  // list (e.g. legacy free-text) follows in first-seen order, and products
  // with no category fall into a trailing "More" bucket.
  const OTHER = "More";
  const categories = useMemo(() => {
    ...
    // Preferred order: managed categories first (in their sort order), then
    // any leftover names in first-seen order, with "More" always last.
    const order: string[] = [];
    for (const c of managedCategories) {
      if (map.has(c.name) && !order.includes(c.name)) order.push(c.name);
    }
    for (const name of seen) {
      if (name !== OTHER && !order.includes(name)) order.push(name);
    }
    if (map.has(OTHER)) order.push(OTHER);
    return order.map((name) => ({ name, items: map.get(name)! }));
  }, [products, managedCategories]);

  // The currently-selected category to browse. Default to the first one and
  // gracefully fall back if the selection disappears (e.g. after a reload).
  const activeCategory = useMemo(() => {
    if (categories.length === 0) return null;
    return categories.find((c) => c.name === activeCat) ?? categories[0];
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:py-12">
      ...
      {products.length === 0 ? (
        ...
        <div>
          {/* Category tiles / pills — pick one to browse its items */}
          <div className="mb-8 flex flex-wrap gap-2.5">
            {categories.map((cat) => {
              const selected = activeCategory?.name === cat.name;
              return (
                <button
                  key={cat.name}
                ...
                >
                  ...
                  <span
                    className={
                      ...
                      (selected ? "bg-white/25 text-white" : "bg-brand-50 text-brand-600")
                    }
                  >
                    {cat.items.length}
                  </span>
          ...
          {activeCategory && (
            <section>
              <div className="mb-5 flex items-center gap-3">
                <h2 className="text-xl font-bold text-brand-700">{activeCategory.name}</h2>
